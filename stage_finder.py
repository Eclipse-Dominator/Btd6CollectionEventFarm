...
def check_page() -> int:
  ...
  '''
  return 0 if find_img(img.expert_page_1_icon) else 1
...
def pos_to_map(pos) -> str:
  ...
  '''
  row = [g_h/4, g_h/2]
  col = [g_w/2 - img.map_box.width/2, g_w/2 + img.map_box.width/2, g_w/2 + img.map_box.width*3/2]
...
def enter_expert_lvl_select() -> bool:
  ...
  '''
  if pos:= find_img(img.enter_level_select_icon):
    click(r2a(pos))

  if pos := find_img(img.toggle_map_icon):
    click(r2a(pos))
    return True
  return False
...
def get_reward_stage(retries = 3) -> tuple[str, tuple[int, int]]:
  ...
  for _ in range(retries):
    sleep(.5)
    if pos := find_img(img.reward_icon):
      return pos_to_map(pos), pos
    else:
      toggle_map_pos = wait_til_exists(img.toggle_map_icon)
...
def play_stage(pos):
  ...
  click(r2a(pos))
  sleep(0.5)
  click(r2a(find_img(img.easy_icon)))
  sleep(0.5)
  
  click(r2a(find_img(img.start_stage_icon)))
  sleep(0.5)

  while not find_img(img.stage_loaded_check_icon):
    if pos := find_img(img.overwrite_ok_icon):
